  ...
  return GoRouter(
    ...
    redirect: (BuildContext context, GoRouterState state) async {
      ...
      if (currentLocation == AppRoutes.splashPath) {
        if (authStateChangesValue is AsyncLoading) {
          return null; 
        }
      ...
      ShellRoute(
        ...
        routes: <RouteBase>[
          GoRoute(
            ...
            routes: [
              GoRoute(
                path: AppRoutes.diagnosisFormPath,
                name: AppRoutes.diagnosisForm,
                parentNavigatorKey: _rootNavigatorKey,
                builder: (context, state) => const DiagnosisFormScreen(),
              ),
              GoRoute(
                path: AppRoutes.imageCaptureGuidePath,
                name: AppRoutes.imageCaptureGuide,
                parentNavigatorKey: _rootNavigatorKey,
                builder: (context, state) => const ImageCaptureGuideScreen(),
              ),
              GoRoute(
                path: '${AppRoutes.diagnosisResultPath}/:reportId',
                name: AppRoutes.diagnosisResult,
                parentNavigatorKey: _rootNavigatorKey,
                builder: (context, state) {
                  final reportId = state.pathParameters['reportId']!;
                  return DiagnosisResultScreen(reportId: reportId);
                },
              ),
            ]
          ),
          GoRoute(
            path: AppRoutes.diagnosisHistoryPath,
            name: AppRoutes.diagnosisHistory,
            pageBuilder: (context, state) => const NoTransitionPage(child: DiagnosisHistoryScreen()),
            routes: [
                GoRoute(
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) => const NoTransitionPage(child: RemindersSettingsScreen()),
          ),
        ],
      ),
    ],
...
class AppRoutes {
  ...
  static const String diagnosisHistory = 'diagnosisHistory';
  static const String diagnosisHistoryPath = '/diagnosis-history';
  static const String diagnosisDetail = 'diagnosisDetail';
  static const String diagnosisDetailPath = 'detail';
  ...
  static const String remindersSettings = 'remindersSettings';
  static const String remindersSettingsPath = '/settings';
}
